...
<div class="eleven wide column">
	<div class="ui green segment">
	    ...
	    <form id="search-form" class="ui form" action="{{ url('profile/hotelcategory/search') }}" method="POST">
    		{{ csrf_field() }}
			<div class="field">
		    ...
		    <a class="ui primary button" href="{{ url('profile/hotelcategory/create') }}">
				...
				<tbody>
					@foreach ($categories as $category)
						<tr>
							...
							<td>{{ $category->name }}</td>
							<td>
								<a class="ui icon button" href="{{ url('profile/hotelcategory/'.$category->id.'/edit') }}">
...
<script type="text/javascript">
	$(document).ready(function() {
	    ...
	    $('#result').on('click', '.open-DeleteModal', function() {
			...
			$('#delete-modal').modal({
				onApprove : function() {
					$('#delete-modal').find('form').attr('action', "{{ url('profile/hotelcategory')}}/" + id);
