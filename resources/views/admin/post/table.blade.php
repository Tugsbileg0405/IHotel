...
<div class="eleven wide column">
	<div class="ui green segment">
	    ...
	    <form id="search-form" class="ui form" action="{{ url('profile/post/search') }}" method="POST">
    		{{ csrf_field() }}
			<div class="field">
				...
				<tbody>
					@foreach ($posts as $post)
						<tr>
							...
							<td>
								<a class="ui icon button" href="{{ url('post', $post->id) }}">
									<i class="eye icon"></i>
								</a>
...
<script type="text/javascript">
	$(document).ready(function() {
	    ...
	    $('#result').on('click', '.open-DeleteModal', function() {
			...
			$('#delete-modal').modal({
				onApprove : function() {
					$('#delete-modal').find('form').attr('action', "{{ url('profile/post')}}/" + id);
