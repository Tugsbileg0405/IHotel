...
<div class="eleven wide column">
	<div class="ui green segment">
		<h4 class="ui header">{{ $category->name }}</h4>
		<div class="ui divider"></div>
	    <form id="search-form" class="ui form" action="{{ url('profile/hotelservice/search') }}" method="POST">
    		{{ csrf_field() }}
			<div class="field">
		    ...
		    <a class="ui button primary" href="{{ url('profile/hotelservice/create', $category->id) }}">
...
<script type="text/javascript">
	$(document).ready(function() {
	    ...
	    $('#result').on('click', '.open-DeleteModal', function() {
			...
			$('#delete-modal').modal({
				onApprove : function() {
					$('#delete-modal').find('form').attr('action', "{{ url('profile/hotelservice')}}/" + id);
