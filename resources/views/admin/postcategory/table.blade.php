...
@section('title', 'Нийтлэлийн ангилал')

@section('content')
<div class="eleven wide column">
	<div class="ui green segment">
		<h4 class="ui header">Нийтлэлийн ангилал</h4>
		<div class="ui divider"></div>
	    <form id="search-form" class="ui form" action="{{ url('profile/postcategory/search') }}" method="POST">
    		{{ csrf_field() }}
			<div class="field">
		    ...
		    <a class="ui button primary" href="{{ url('profile/postcategory/create') }}">
				...
				<tbody>
					@foreach ($categories as $category)
						<tr>
							...
							<td>{{ $category->name }}</td>
							<td>
								<a class="ui icon button" href="{{ url('profile/postcategory/'.$category->id.'/edit') }}">
...
<script type="text/javascript">
	$(document).ready(function() {
	    ...
	    $('#result').on('click', '.open-DeleteModal', function() {
			...
			$('#delete-modal').modal({
				onApprove : function() {
					$('#delete-modal').find('form').attr('action', "{{ url('profile/postcategory')}}/" + id);
